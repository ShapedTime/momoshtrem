...
type FallbackHandler interface {
	IdentifyBatch(files []UnidentifiedFile, metadata *loader.TMDBMetadata) (map[string]*IdentifiedFile, error)
}
...
func (n *NoOpFallback) IdentifyBatch(files []UnidentifiedFile, metadata *loader.TMDBMetadata) (map[string]*IdentifiedFile, error) {
	return nil, nil
}
...
func (i *Identifier) Identify(metadata *loader.TMDBMetadata, files []TorrentFile, torrentName string) *IdentificationResult {
	...
	if len(result.UnidentifiedFiles) > 0 && i.fallback != nil {
		...
		fallbackResults, err := i.fallback.IdentifyBatch(unidentified, metadata)
